...
const Components = {
    ...
    bindChatListKeyboardNav() {
        ...
        list.addEventListener('keydown', (e) => {
            ...
            switch (e.key) {
                case 'ArrowDown':
                    e.preventDefault();
                    newIdx = Math.min(idx + 1, items.length - 1);
                    break;
                case 'ArrowUp':
                    e.preventDefault();
    ...
    _syncVisibleTabindex() {
        const items = [...this.chatList.querySelectorAll('.chat-item:not([hidden])')];
    ...
    loadChats() {
        let saved;
        try {
            saved = localStorage.getItem('prism_chats');
            if (saved) {
            ...
            Utils.showNotification('Chat data was corrupted and has been reset.');
        }
    ...
    saveChats() {
        try {
            ...
            Utils.showNotification('Failed to save chat data. Storage may be full.');
        }
    },
    ...
    renderChatList(filter = '') {
        ...
        this.chats.forEach((chat, index) => {
            ...
            chatItem.setAttribute('tabindex', index === 0 ? '0' : '-1');
    ...
    showSettingsModal() {
        const existingModal = Utils.$('#settingsModal');
        if (existingModal) {
            existingModal.remove();
        }

        const storedTheme = localStorage.getItem('prism-theme') || 'dark';
        ...
        const themeSection = Utils.createElement('div', {
            className: 'settings-section'
        });

        const themeLabel = Utils.createElement('span', {
            className: 'settings-label',
            textContent: 'Theme'
        });

        const themeOptions = Utils.createElement('div', {
        ...
        const setTheme = (value) => {
            ...
            themeOptions.querySelectorAll('.theme-option').forEach(b => {
                b.classList.toggle('active', b.value === value);
                b.setAttribute('aria-checked', b.value === value ? 'true' : 'false');
            });

            document.documentElement.dataset.theme = value;
            localStorage.setItem('prism-theme', value);
        ...
        themes.forEach(t => {
            const btn = Utils.createElement('button', {
                className: `theme-option${storedTheme === t.value ? ' active' : ''}`,
                role: 'radio',
                'aria-checked': storedTheme === t.value ? 'true' : 'false',
                'aria-label': t.label,
                value: t.value
            });
            ...
            if (t.value === 'light') {
                ...
                p1.setAttribute('cx', '12'); p1.setAttribute('cy', '12'); p1.setAttribute('r', '5');
                svg.appendChild(p1);
                const p2 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                p2.setAttribute('d', 'M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42');
                svg.appendChild(p2);
            } else {
                const p1 = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                p1.setAttribute('d', 'M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z');
                svg.appendChild(p1);
            }

            btn.appendChild(svg);

            const label = Utils.createElement('span', { textContent: t.label });
            btn.appendChild(label);

            btn.addEventListener('click', () => setTheme(t.value));
        ...
        const notificationsSection = Utils.createElement('div', {
            className: 'settings-section'
        });

        const notifLabel = Utils.createElement('span', {
            className: 'settings-label',
            textContent: 'Notifications'
        });

        const notifRow = Utils.createElement('div', {
            className: 'toggle-row'
        });

        const notifText = Utils.createElement('span', {
            textContent: 'Message previews'
        });

        const notifToggle = Utils.createElement('button', {
            className: 'toggle active',
            role: 'switch',
            'aria-checked': 'true',
            'aria-label': 'Toggle message previews'
        });

        notifRow.appendChild(notifText);
        notifRow.appendChild(notifToggle);

        const notifDesc = Utils.createElement('p', {
            className: 'settings-description',
            textContent: 'Show message content in notifications.'
        });

        notificationsSection.appendChild(notifLabel);
        notificationsSection.appendChild(notifRow);
        notificationsSection.appendChild(notifDesc);

        const aboutSection = Utils.createElement('div', {
            className: 'settings-section'
        });

        const aboutLabel = Utils.createElement('span', {
            className: 'settings-label',
            textContent: 'About'
        });

        const aboutText = Utils.createElement('p', {
            className: 'settings-about',
            textContent: 'Prism v1.0.0 — Secure Instant Messaging. Built with a focus on privacy and user experience.'
        });
        ...
        const triggerElement = Utils.$('#menuBtn');
        const focusableSelector = 'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';
        const getFocusable = () => [...modal.querySelectorAll(focusableSelector)].filter(el => !el.disabled);
        ...
        notifToggle.addEventListener('click', () => {
            const isActive = notifToggle.classList.toggle('active');
            notifToggle.setAttribute('aria-checked', isActive);
        });

        const firstOption = themeOptions.querySelector('.theme-option');
